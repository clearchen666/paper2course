...
### Missing Bilingual Switch
Paper2Course outputs must include both `index.html` and `index.zh.html`. Each page must show the language switch in the top nav, with the current language visually active and the other language linked. Verify the relative links work when opened as local files.

### Missing Closing Encouragement
The last screen of the final module should include a specific encouragement note. It should not be generic praise; it should name what the learner can now do after reading the paper and reinforce a confident, skeptical reading habit.

### Missing Final Synthesis Or FAQ
The learner should not leave with only details. Before the encouragement note, include a high-level synthesis and a short FAQ that resolves the paper's likely category confusions: what is trained once, what changes per user or corpus, where memory is stored, how it differs from the obvious baseline, and what a skeptical builder should test first.
